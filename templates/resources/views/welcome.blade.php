...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="flex flex-col h-screen antialiased">
        @if (Route::has('login'))
            <header class="container flex mx-auto font-bold text-gray-500">
                <ul class="flex ml-auto space-x-4">
                    @auth
                        <li><a class="flex px-4 py-4 uppercase hover:text-gray-900" href="{{ url('/home') }}">Home</a></li>
                    @else
                        <li><a class="flex px-4 py-4 uppercase hover:text-gray-900" href="{{ route('login') }}">Login</a></li>

                        @if (Route::has('register'))
                            <li><a class="flex px-4 py-4 uppercase hover:text-gray-900" href="{{ route('register') }}">Register</a></li>
                        @endif
                    @endauth
                </ul>
            </header>
